...
{
	//printk("vgic_int_has_other_target %d\n", vcpu->id);
...
{
	//printk("vgicd_emul_sgiregs_access %d\n", handlers->regid);
...
{
	//printk("vgic_inject_sgi %d %d %d\n", vcpu->id, interrupt, source);
	...
	//printk("> %u %u\n", pendstate, new_pendstate);
	
    if (pendstate ^ new_pendstate) {
		//printk("pendstate ^ new_pendstate\n");
