...
{
	//printk("vgicd_emul_misc_access %d\n", handlers->regid);
...
void vgicd_emul_pidr_access(struct emul_access *acc,
                            ...
                            bool gicr_access, cpuid_t vgicr_id)
{
	//printk("vgicd_emul_pidr_access %d\n", handlers->regid);
...
{
	//printk("vgic_int_update_enable %d\n", vcpu->id);
...
{
	//printk("vgic_int_clear_enable %d\n", vcpu->id);
...
void vgic_emul_generic_access(struct emul_access *acc,
                              ...
                              bool gicr_access, cpuid_t vgicr_id)
{
	//printk("vgic_emul_generic_access %d\n", handlers->regid);
...
void vgic_inject_hw(struct vcpu* vcpu, irqid_t id) {
	//printk("vgic_inject_hw %d\n", id);

	/* task_limit++; */
	/* if (task_limit == 400) */
	/* { */
	/* 	task_limit = 0; */
	/* 	vcpu_inject_irq(cpu()->vcpu, 1); */
	/* } */
...
void vgic_inject(struct vcpu* vcpu, irqid_t id, vcpuid_t source)
{
    ...
    if (interrupt != NULL) {
        if (vgic_int_is_hw(interrupt)) {
			//printk("vgic_int_is_hw(interrupt)\n");
            vgic_inject_hw(vcpu, id);
        } else if (GIC_VERSION == GICV2 && gic_is_sgi(id)) {
			//printk("GIC_VERSION == GICV2 && gic_is_sgi(id)\n");
            vgic_inject_sgi(vcpu, interrupt, source);
        } else {
			//printk("GIC_VERSION == GICV2 && gic_is_sgi(id) -> false\n");
...
{
	//printk("gic_maintenance_handler %d\n", irq_id);
