...
void gic_pmu_handler()
{
	//printk("gic_pmu_handler()\n");
}

void gicd_init()
{
	//printk("BAO gicd_init()\n");
...
{	
	...
	//printk("gicd_get_prio %d %d\n", int_id, prio);
    return prio;
}

void gicd_set_icfgr(irqid_t int_id, uint8_t cfg)
{	
	//printk("gicd_set_icfgr %d\n", int_id);
