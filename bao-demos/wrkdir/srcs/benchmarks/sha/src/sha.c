...
void sha_stream(SHA_INFO *sha_info)
{
	...
	int i;
    char data[BLOCK_SIZE];  // Buffer
	
	for (i = 0; i < len; i += BLOCK_SIZE)
	{
        size_t chunk_size = (i + BLOCK_SIZE <= len) ? BLOCK_SIZE : len - i;
        memcpy(data, &content[i], chunk_size);
        sha_update(NULL, data, chunk_size);
    }
...
BYTE content[] =
{
	"KurtVonnegutsCommencementAddressatMITLadiesandgentlemenoftheclassof97Wearsunscreen"
	"IfIcouldofferyouonlyonetipforthefuturesunscreenwouldbeitThelongtermbenefitsofsunscreen"
	"havebeenprovedbyscientistswhereastherestofmyadvicehasnobasismorereliablethanmyownmeanderingexperience"
	"IwilldispensethisadvicenowEnjoythepowerandbeautyofyouryouthOhnevermindYouwillnotunderstandthepowerand"
	"beautyofyouryouthuntiltheyvefadedButtrustmein20yearsyoulllookbackatphotosofyourselfandrecallinawayyou"
	"KurtVonnegutsCommencementAddressatMITLadiesandgentlemenoftheclassof97Wearsunscreen"
	"IfIcouldofferyouonlyonetipforthefuturesunscreenwouldbeitThelongtermbenefitsofsunscreen"
	"havebeenprovedbyscientistswhereastherestofmyadvicehasnobasismorereliablethanmyownmeanderingexperience"
	"IwilldispensethisadvicenowEnjoythepowerandbeautyofyouryouthOhnevermindYouwillnotunderstandthepowerand"
